  ...
  const loadInitialState = () => {
    try {
      ...
      if (savedState) {
        ...
        delete savedConfig.istsosAdminPassword;
        delete savedConfig.secretKey;
  ...
  useEffect(() => {
    ...
    saveTimeoutRef.current = setTimeout(() => {
      try {
        ...
        delete stateToSave.configuration.postgresPassword;
        delete stateToSave.configuration.istsosAdminPassword;
        delete stateToSave.configuration.secretKey;
  ...
  useEffect(() => {
    const handleStorageChange = (e) => {
      if (e.key === STORAGE_KEY && e.newValue && e.newValue !== e.oldValue) {
        if (
          window.confirm(
            "The wizard has been updated in another tab. Do you want to load those changes?"
          )
        ) {
